...
`STRIMZI_KAFKA_IMAGE_MAP`:: Required.
This provides a mapping from Kafka version to the corresponding Docker image containing a Kafka broker of that version. The required syntax is whitespace separated `_<version>_=_<image>_` pairs.
This is used when a `Kafka.spec.kafka.version` property is specified but not the `Kafka.spec.kafka.image`, as descrbied in xref:assembly-configuring-container-images-deployment-configuration-kafka[].
